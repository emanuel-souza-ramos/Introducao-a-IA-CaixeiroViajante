...
def run_experiment(SaClass, name, city_dist_mat, T_0, alpha, patience, num_runs=10):
    print("\n" + "=" * 60)
    ...
    for i in range(num_runs):
        ...
        # Redirecionamos temporariamente o stdout para silenciar os logs da execução interna
...
def main():
    ...
    # Executa os experimentos (10 rodadas de cada versão de SA)
    num_runs = 10
    stats = {}
    stats["SA V1"] = run_experiment(SaV1, "SA V1", city_dist_mat, T_0=1000, alpha=0.95, patience=300, num_runs=num_runs)
    stats["SA V2"] = run_experiment(SaV2, "SA V2", city_dist_mat, T_0=1000, alpha=0.98, patience=300, num_runs=num_runs)
    stats["SA V3"] = run_experiment(SaV3, "SA V3", city_dist_mat, T_0=500, alpha=0.90, patience=150, num_runs=num_runs)

    # Imprime tabela de logs estatísticos
    print("\n" + "=" * 90)
    print(f"RELATÓRIO ESTATÍSTICO COMPILADO SA (MÉDIA DE {num_runs} EXECUÇÕES) - MAPA: {os.path.basename(selected_file)}")
    print("=" * 90)
    ...
    # Plot 1: Curva de Convergência Média (Fitness médio vs. Iteração)
    plt.figure(figsize=(10, 6))
    plt.plot(stats["SA V1"]["average_curve"], color='#1f77b4', linewidth=2.5, label='SA V1 (T0:1000, a:0.95)')
    plt.plot(stats["SA V2"]["average_curve"], color='#ff7f0e', linewidth=2.5, label='SA V2 (T0:1000, a:0.98)')
    plt.plot(stats["SA V3"]["average_curve"], color='#2ca02c', linewidth=2.5, label='SA V3 (T0:500, a:0.90)')
    plt.title(f"Curva de Convergência Média SA ({num_runs} Execuções) - {map_name}", fontsize=12, fontweight='bold')
    plt.xlabel("Iteração")
    ...
    axes[0].plot(run_indices, stats["SA V1"]["fitness_values"], 'o-', color='#1f77b4', label='SA V1')
    axes[0].plot(run_indices, stats["SA V2"]["fitness_values"], 's-', color='#ff7f0e', label='SA V2')
    axes[0].plot(run_indices, stats["SA V3"]["fitness_values"], '^-', color='#2ca02c', label='SA V3')
    # Adiciona linhas pontilhadas com a média
    axes[0].axhline(stats["SA V1"]["avg_fitness"], color='#1f77b4', linestyle='--', alpha=0.7)
    axes[0].axhline(stats["SA V2"]["avg_fitness"], color='#ff7f0e', linestyle='--', alpha=0.7)
    axes[0].axhline(stats["SA V3"]["avg_fitness"], color='#2ca02c', linestyle='--', alpha=0.7)
    axes[0].set_title("Distância Mínima Encontrada por Rodada SA", fontsize=11, fontweight='bold')
    ...
    axes[1].plot(run_indices, stats["SA V1"]["times"], 'o-', color='#1f77b4', label='SA V1')
    axes[1].plot(run_indices, stats["SA V2"]["times"], 's-', color='#ff7f0e', label='SA V2')
    axes[1].plot(run_indices, stats["SA V3"]["times"], '^-', color='#2ca02c', label='SA V3')
    # Adiciona linhas pontilhadas com a média
    axes[1].axhline(stats["SA V1"]["avg_time"], color='#1f77b4', linestyle='--', alpha=0.7)
    axes[1].axhline(stats["SA V2"]["avg_time"], color='#ff7f0e', linestyle='--', alpha=0.7)
    axes[1].axhline(stats["SA V3"]["avg_time"], color='#2ca02c', linestyle='--', alpha=0.7)
    axes[1].set_title("Tempo de Execução por Rodada SA", fontsize=11, fontweight='bold')
    ...
    axes[2].plot(run_indices, stats["SA V1"]["memories"], 'o-', color='#1f77b4', label='SA V1')
    axes[2].plot(run_indices, stats["SA V2"]["memories"], 's-', color='#ff7f0e', label='SA V2')
    axes[2].plot(run_indices, stats["SA V3"]["memories"], '^-', color='#2ca02c', label='SA V3')
    # Adiciona linhas pontilhadas com a média
    axes[2].axhline(stats["SA V1"]["avg_memory"], color='#1f77b4', linestyle='--', alpha=0.7)
    axes[2].axhline(stats["SA V2"]["avg_memory"], color='#ff7f0e', linestyle='--', alpha=0.7)
    axes[2].axhline(stats["SA V3"]["avg_memory"], color='#2ca02c', linestyle='--', alpha=0.7)
    axes[2].set_title("Pico de Consumo de Memória por Rodada SA", fontsize=11, fontweight='bold')
